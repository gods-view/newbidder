...
class BaseManager(models.Manager):
    ...
    def get_all_object(self):
        try:
            # self.get(username=username, password=password)
            # obj = self.filter(id__lt=20*1024).order_by('id')
            obj = self.all().order_by('id')[0:100]
        except self.model.DoesNotExist:
            print("运行出错")
            obj = None
        return obj
    ...
    def get_object_list(self, filters={}, order_by=('id',)):
        '''根据条件获取查询集并排序'''
    ...
    # filters = {'goods_type_id':goods_type_id, order_by=order_by)
    def get_object_list_by_condition(self, filters={}, order_by=('id',)):
        '''根据条件获取查询集并排序'''
        # self.filter(goods_type_id=goods_type_id).order('-pk',)
        object_list = self.filter(
            models.Q(offerId__icontains=filters["offerId"]) | models.Q(offerId__icontains=filters["offerId"])).order_by(
            *order_by)
        return object_list
